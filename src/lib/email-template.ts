import type { Parcela, ParcelasSummary } from "@/lib/report-data";
...
function fmt(val: string | null | undefined, fallback = "&mdash;"): string {
  return val ?? fallback;
}
...
function summaryTable(summary: ParcelasSummary): string {
  const rows = ALL_ESTADOS.map((estado) => {
    ...
    const style: { bg: string; color: string; border: string } =
      ESTADO_PARCELA_STYLES[estado] ?? { bg: "#f4f4f5", color: "#374151", border: "#e4e4e7" };
...
function reservationsTable(reservations: Parcela[]): string {
  if (reservations.length === 0) {
    return `<p style="margin:0;color:#9ca3af;font-style:italic;font-size:14px;">Sin reservas en los &uacute;ltimos 7 d&iacute;as.</p>`;
  }
...
export function buildDailyReportHtml(data: {
  ...
  const { date, summary, appUrl, reservations } = data;
...
<head>
  ...
  <title>Reporte Diario &mdash; Barrio Stefani</title>
</head>
    ...
    <tr>
      <td align="center">
        <table width="680" cellpadding="0" cellspacing="0" style="max-width:680px;width:100%;background:#ffffff;border-radius:10px;border:1px solid #e4e4e7;overflow:hidden;">
          ...
          <tr>
            <td style="padding:28px 32px 0;">
              <h2 style="margin:0 0 4px;color:#0f172a;font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;">Reservas de los &uacute;ltimos 7 d&iacute;as</h2>
          ...
          <tr>
            <td style="padding:32px;text-align:center;">
              ...
              <p style="margin:0;color:#9ca3af;font-size:12px;line-height:1.6;">Generado autom&aacute;ticamente por <strong style="color:#6b7280;">Barrio Stefani CRM</strong></p>
